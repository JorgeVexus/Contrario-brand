    ...
    class QuickOrderList extends BulkAdd {
      ...
      initVariantEventListeners() {
        ...
        this.querySelectorAll('quantity-input').forEach((qty) => {
          ...
          qty.addEventListener('change', (event) => {
            ...
            this.toggleLoading(true, event.target);
      ...
      updateMultipleQty(items) {
        ...
        fetch(`${routes.cart_update_url}`, { ...fetchConfig(), ...{ body } })
          ...
          .finally(() => {
            if (this.queue.length === 0) this.toggleLoading(false);
      ...
      toggleLoading(loading, target = this) {
        this.querySelector('#shopping-cart-variant-item-status')?.toggleAttribute('aria-hidden', !loading);
        this.toggleRowLoading(target, loading);
        this.toggleTotalsLoading(loading);

        this.querySelectorAll('.variant-remove-total .loading__spinner')?.forEach((spinner) => {
          spinner.classList.toggle('hidden', !loading);
        });
      }

      toggleRowLoading(target, loading) {
        if (!loading) {
          this.querySelectorAll('.variant-item[aria-busy="true"]').forEach((row) => {
            row.removeAttribute('aria-busy');
            row.querySelectorAll('.quantity__button, .quantity__input').forEach((control) => {
              control.removeAttribute('disabled');
            });
            row.querySelectorAll('.variant-item__totals .loading__spinner').forEach((spinner) => {
        ...
        const row = target?.closest?.('.variant-item');
        if (!row) return;
      ...
      toggleTotalsLoading(loading) {
        const total = this.getTotalBar();
        if (!total) return;

        total.toggleAttribute('aria-busy', loading);
        total.querySelector('.quick-order-list__total-loading')?.classList.toggle('hidden', !loading);
      }
    }
  );
}
